...
const CITY_MAP = { riyadh: 'الرياض', jeddah: 'جدة', dammam: 'الدمام' };
...
const TABS = [
  ...
  { id: 'contact', label: 'الاتصال والتواصل', icon: '📞' },
    ...
    return (
      <>
        ...
        <Section title="بيانات الطلب">
          <DataRow label="نوع التأشيرة" value={VISA_TYPE_MAP[app.visaType] || app.visaType} />
          <DataRow label="مدينة التقديم" value={CITY_MAP[app.applicationCity] || app.applicationCity} />
        </Section>
  ...
  const renderFinancialTab = () => {
    ...
    return (
      <>
        ...
        {e.hasPreviousJob && (
          (e.previousJobs && e.previousJobs.length > 0) ? (
            e.previousJobs.map((job, idx) => (
              <Section key={idx} title={`الوظيفة السابقة ${idx + 1}`}>
                <DataRow label="جهة العمل السابقة" value={job.employerName} />
                <DataRow label="عنوان جهة العمل السابقة" value={job.employerAddress} />
                <DataRow label="المسمى الوظيفي السابق" value={job.jobTitle} />
                <DataRow label="اسم المدير السابق" value={job.managerName} />
                <DataRow label="تاريخ بدء العمل السابق" value={formatDate(job.startDate)} />
                <DataRow label="تاريخ انتهاء العمل السابق" value={formatDate(job.endDate)} />
              </Section>
            ))
          ) : (
  ...
  const renderEducationTab = () => {
    ...
    return (
      <>
        ...
        {ed.hasEducation && (
          (ed.records && ed.records.length > 0) ? (
            ed.records.map((rec, idx) => (
              <Section key={idx} title={`الملف الدراسي ${idx + 1}`}>
                <DataRow label="اسم الجامعة" value={rec.universityName} />
                <DataRow label="التخصص" value={rec.major} />
                <DataRow label="مدينة الجامعة" value={rec.universityCity} />
                <DataRow label="الدولة" value={rec.country} />
                <DataRow label="عنوان الجامعة" value={rec.universityAddress} />
                <DataRow label="تاريخ بدء الدراسة" value={formatDate(rec.studyStartDate)} />
                <DataRow label="تاريخ التخرج" value={formatDate(rec.graduationDate)} />
              </Section>
            ))
          ) : (
            <Section title="الملف الدراسي">
    ...
    return (
      <>
        <Section title="الدول المزارة خلال 5 سنوات">
          <DataRow label="زار دولاً خلال 5 سنوات؟" value={displayBool(th.hasVisitedCountries)} />
